...
LOG_PATH = os.path.join(LOGS_DIR, "media_control.log")
...
class MediaControlApp:
        ...
        try:
            if msg == WM_TRAY:
                if lparam == win32con.WM_LBUTTONDOWN:
                    ...
                    logger.info(f"Tray icon clicked. Control ID: {wparam}, Active Sessions Count: {count}")
    ...
    def handle_play_pause(self):
        playing_sessions = [s for s in getattr(self, 'active_sessions', []) if s["status"] == 4]
        if len(playing_sessions) > 1:
            ...
            logger.info("Single session playing/paused or all paused. Sending global play/pause.")
        ...
        while True:
            ...
            try:
                ...
                if session:
                    ...
                    if (status != self.current_status or 
                    ...
                    if (self.current_status is not None or 
                ...
                try:
                    ...
                    for s in pycaw_sessions:
                        try:
                            ...
                            if any(x in lower_name for x in ["svchost", "health_app", "python", "antigravity", "system", "idle"]):
                                continue
                                
                            # ONLY fallback for known media players that break SMTC (e.g. VLC, MPC-HC, iTunes)
                            known_players = ["vlc", "mpc-hc", "itunes", "winamp", "wmplayer", "potplayermini64"]
                            ...
                            for d in active_sessions_list:
                                ...
                                if friendly.lower() in d_friendly.lower() or d_friendly.lower() in friendly.lower():
                                    already_in_smtc = True
                                    break
                                    
                            if not already_in_smtc:
                                status = 4 if s.State == 1 else 5 # 1=Active(Playing), 0=Inactive(Paused)
    ...
    def update_tooltips(self, status, title="", artist=""):
        ...
        status_desc = "Playing" if status == 4 else "Paused" if status in (3, 5) else "Idle"
        ...
        for ctrl in self.controls:
            tip = play_tip if ctrl["type"] in ("play", "pause") else prev_tip if ctrl["type"] == "prev" else next_tip
            ctrl["tip"] = tip
            try:
    ...
    def send_pycaw_command(self, process_name, command):
        ...
        try:
            ...
            target_pids = set(p.pid for p in psutil.process_iter(['name']) if p.info['name'] and p.info['name'].lower() == process_name.lower())
