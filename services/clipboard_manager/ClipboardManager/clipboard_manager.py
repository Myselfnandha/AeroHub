...
DEFAULT_CONFIG = {"max_entries": 1000, "export_batch": 500, "auto_export": True}
...
class ClipboardDB:
    ...
    def get_entries(self, search: str = "", limit: int = 500) -> list:
        ...
        with self.lock:
            if search:
                query = (
                    "SELECT id, content_type, content, timestamp "
                    "FROM clipboard_history WHERE content LIKE ? "
    ...
    def delete_ids(self, ids: list):
        with self.lock:
            ...
            self.conn.execute(
                f"DELETE FROM clipboard_history WHERE id IN ({placeholders})", ids
            )
            self.conn.commit()
...
class ClipboardManagerGUI:
    ...
    def open_settings(self):
        ...
        auto_cb = tk.Checkbutton(
            auto_frame,
            text="Enable Auto-Export to Markdown",
            variable=auto_var,
            font=("Segoe UI", 10),
            bg=THEME["bg"],
            fg=THEME["fg"],
    ...
    def _on_select(self, event):
        ...
        self.preview_text.insert(
            "1.0", f"[{timestamp}] — {content_type.upper()}\n\n{content}"
        )
    ...
    def _clear_all(self):
        if messagebox.askyesno(
            "Clear All", "Delete all clipboard history?", parent=self.root
        ):
            self.db.clear_all()
            self.refresh_list()
    ...
    def _manual_export(self):
        ...
        with open(md_path, "w", encoding="utf-8") as f:
            ...
            for entry_id, content_type, content, timestamp in entries:
                f.write(
                    f"## [{timestamp}] — {content_type.upper()}\n\n```\n{content}\n```\n\n---\n\n"
                )
...
class ClipboardManagerApp:
    ...
    def _on_quit(self, icon, item):
        ...
        self._running = False
        icon.stop()
        self._emergency_save_hook()
        self.db.close()
        os._exit(0)

    def _emergency_save_hook(self, ctrl_type=None):
        try:
            win32clipboard.OpenClipboard()
            try:
                if win32clipboard.IsClipboardFormatAvailable(win32con.CF_UNICODETEXT):
                    data = win32clipboard.GetClipboardData(win32con.CF_UNICODETEXT)
                    if data and data.strip():
                        self.db.add_entry("text", data.strip())
                elif win32clipboard.IsClipboardFormatAvailable(CF_HDROP):
                    data = win32clipboard.GetClipboardData(CF_HDROP)
                    if data:
                        file_paths = (
                            "\n".join(data)
                        ...
                        self.db.add_entry("filepath", file_paths)
            finally:
                win32clipboard.CloseClipboard()
        except Exception:
            pass
    ...
    def run(self):
        ...
        atexit.register(self._emergency_save_hook)
        try:
            win32api.SetConsoleCtrlHandler(self._emergency_save_hook, True)
        except Exception as e:
            logger.error(f"Failed to set console ctrl handler: {e}")
