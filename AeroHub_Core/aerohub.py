...
LOG_PATH = os.path.join(LOGS_DIR, "aerohub.log")
...
DEFAULT_CONFIG = {
    ...
    "processes": [
        ...
        },
        {
            ...
            "name": "Health App",
            "icon": "👁️",
            "script": "HealthApp/health_app.py",
            "auto_start": True,
            "enabled": True,
        },
        {
            ...
            "name": "Media Control",
            "icon": "🎵",
            "script": "MediaControl/media_control.py",
            "auto_start": True,
            "enabled": True,
        },
        {
            ...
            "icon": "🔋",
            "script": "BatteryMonitor/battery_monitor.py",
            "auto_start": True,
            "enabled": True,
        },
        {
            ...
            "name": "Temp Monitor",
            "icon": "🌡️",
            "script": "TempMonitor/temp_monitor.py",
            "auto_start": True,
            "enabled": True,
        },
        {
            ...
            "name": "Touch Toggle",
            "icon": "👆",
            "script": "TouchToggle/touch_toggle.py",
            "auto_start": False,
            "enabled": True,
        },
        {
            ...
            "icon": "📡",
            "script": "TgFdmProxy/tg_fdm_proxy.py",
            "auto_start": False,
            "enabled": True,
        },
        {
            ...
            "icon": "🔊",
            "script": "TaskbarScroll/taskbar_scroll.py",
            "auto_start": True,
            "enabled": True,
        },
    ],
...
class DashboardWidget:
    ...
    def __init__(self, processes: list, on_toggle, on_restart, on_start_all=None, on_stop_all=None, on_exit=None):
    ...
    def create(self):
        ...
        def add_hover(widget, hover_fg):
            ...
            widget.bind("<Enter>", lambda e: widget.config(bg=TH["bg_card"], fg=hover_fg))
            widget.bind("<Leave>", lambda e: widget.config(bg=TH["bg2"], fg=default_fg))

        if self.on_start_all:
            btn = tk.Button(
                title_bar, text="▶", font=("Segoe UI", 10),
                bg=TH["bg2"], fg=TH["running"], activebackground=TH["bg_card"], activeforeground=TH["running"],
                relief=tk.FLAT, cursor="hand2", command=self.on_start_all
            )
        ...
        if self.on_stop_all:
            btn = tk.Button(
                title_bar, text="■", font=("Segoe UI", 10),
                bg=TH["bg2"], fg=TH["danger"], activebackground=TH["bg_card"], activeforeground=TH["danger"],
                relief=tk.FLAT, cursor="hand2", command=self.on_stop_all
            )
            btn.pack(side=tk.LEFT, padx=2)
        ...
        if self.on_exit:
            btn = tk.Button(
                title_bar, text="⏻", font=("Segoe UI", 10),
                bg=TH["bg2"], fg=TH["fg_dim"], activebackground=TH["bg_card"], activeforeground=TH["accent"],
                relief=tk.FLAT, cursor="hand2", command=self.on_exit
            )
            btn.pack(side=tk.LEFT, padx=2)
        ...
        for i, proc in enumerate(self.processes):
            ...
            tk.Label(
                ...
                bg=TH["bg_card"], fg=TH["fg"], width=24, anchor=tk.W
            ).pack(side=tk.LEFT)
            ...
            if temp_monitor_paused:
                for proc in self.processes:
                    if proc.id == "temp_monitor" and proc.status == "running":
                        logger.info("[GAME MODE] temp_monitor manually started by user. Overriding pause.")
                        temp_monitor_paused = False
...
def check_admin_and_elevate():
    ...
    logger.warning("Not running as Administrator — attempting self-elevation...")
    try:
    ...
    # If elevation failed or was cancelled, warn user
    import tkinter as tk
    ...
    proceed = messagebox.askyesno(
        "Privilege Warning",
        "AeroHub was not elevated to Administrator.\n"
        "Some features (like Touch Toggle) require admin privileges and will not work.\n\n"
    ...
    if not proceed:
        sys.exit(0)
    logger.warning("Proceeding WITHOUT admin privileges (user chose to continue).")
...
if __name__ == "__main__":
    try:
        ...
        import traceback
        with open(os.path.join(LOGS_DIR, "aerohub_crash.log"), "w", encoding="utf-8") as f:
