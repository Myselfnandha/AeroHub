...
$script = Join-Path $repo "services\aerohub_core\aerohub.py"
...
if ($action -eq "install") {
    ...
    & $nssmPath set $serviceName AppStdout "$(Join-Path $repo 'Logs\aerohub.stdout.log')"
    & $nssmPath set $serviceName AppStderr "$(Join-Path $repo 'Logs\aerohub.stderr.log')"
