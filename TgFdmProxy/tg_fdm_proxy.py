...
from telethon.tl.types import (
    ...
    KeyboardButtonRequestPeer, RequestPeerTypeBroadcast,
    ReplyKeyboardMarkup, KeyboardButtonRow,
    MessageActionRequestedPeerSentMe, RequestedPeerChannel,
    PeerChannel
)
...
LOG_FILE_PATH = os.path.join(LOGS_DIR, "tg_fdm_proxy.log")
...
client = TelegramClient(
    os.path.join(os.path.dirname(SCRIPT_DIR), "Logs", "fdm_proxy_bot_session"), API_ID, API_HASH,
...
async def _download_chunk_task(media, offset, limit, chunk_size, max_retries):
    ...
    for attempt in range(max_retries + 1):
        try:
            chunks = []
            async for c in client.iter_download(media, offset=offset, limit=limit, chunk_size=chunk_size):
                if c:
                    chunks.append(c)
            return b"".join(chunks)
        except Exception as e:
            if attempt == max_retries:
                logger.error(f"[DOWNLOAD] Chunk at offset {offset} failed after {max_retries} retries: {e}")
                raise e
            logger.warning(f"[DOWNLOAD] Chunk at offset {offset} failed (attempt {attempt + 1}/{max_retries + 1}): {e}. Retrying...")
...
async def handle_download(request: web.Request) -> web.StreamResponse:
    ...
    try:
        ...
        if not message or not message.media or not hasattr(message, "file"):
            add_event("ERROR", f"File not found: chat {chat_id}, msg {message_id}", "error")
        ...
        if range_header:
            ...
            if start >= file_size:
                return web.Response(status=416, text="Requested Range Not Satisfiable", headers={
        ...
        if DOWNLOAD_METHOD == "parallel":
            ...
            while curr_offset < aligned_offset + length + bytes_to_skip:
                curr_limit = min(chunk_size, aligned_offset + length + bytes_to_skip - curr_offset)
            ...
            for idx in range(min(max_concurrency - 1, total_chunks)):
                ...
                active_tasks[idx] = asyncio.create_task(
                    _download_chunk_task(message.media, off, lim, chunk_size, max_retries)
                )
            ...
            try:
                for idx in range(total_chunks):
                    ...
                    if next_to_schedule < total_chunks and next_to_schedule not in active_tasks:
            ...
            if _key in download_registry:
                ...
                if total_unique_bytes >= reg["size_bytes"]:
                    ...
                    if not reg.get("opened", False):
                        reg["opened"] = True
                        logger.info(f"[DOWNLOAD] Opening download folder automatically for {file_name}")
                        add_event("DOWNLOAD", f"Opening download folder for {file_name}")
                        try:
                            if os.path.exists(DOWNLOAD_DIR):
                                os.startfile(DOWNLOAD_DIR)
                        except Exception as err:
                            logger.error(f"[DOWNLOAD] Failed to open download folder: {err}")

                    # 2. Dispatch speed-stats Telegram reply (exactly once)
                    if not reg.get("notified", False):
                        reg["notified"] = True
                        _elapsed  = time.monotonic() - reg["start"]
                        _speed_mb = reg["size_bytes"] / max(_elapsed, 0.1) / (1024 * 1024)
                        _size_gb  = reg["size_bytes"] / (1024 ** 3)
                        _mins, _secs = divmod(int(_elapsed), 60)
                        _time_str = f"{_mins}m {_secs}s" if _mins else f"{_secs}s"

                        # Log finished stats
                        add_event("DOWNLOAD", f"Finished {file_name} — {_size_gb:.2f} GB in {_time_str} (~{_speed_mb:.1f} MB/s)")

                        if reg.get("reply_chat"):
                            async def _send_stats(_i=reg, _ts=_time_str, _sm=_speed_mb, _sg=_size_gb):
                                try:
        ...
        add_event("ERROR", f"Download failed for {chat_id}/{message_id}: {str(e)}", "error")
...
async def cmd_add_channel(event):
    ...
    if not arg:
        ...
        await event.reply("Tap the button below to choose a channel to watch, or type `/add_channel <username/ID>`:", buttons=buttons)
        return
    arg = arg.strip()
    ...
    save_config_to_env({"TARGET_CHANNELS": ",".join(str(ch) for ch in sorted(ACTIVE_CHANNELS))})
...
async def cmd_remove_channel(event):
    ...
    if not arg:
        ...
        await event.reply("Tap the button below to choose a channel to stop watching, or type `/remove_channel <username/ID>`:", buttons=buttons)
        return
    arg = arg.strip()
    channel = int(arg) if arg.lstrip("-").isdigit() else arg
...
async def _sniffer_handler(event):
    ...
    if ALLOWED_EXT:
        ...
        if _ext not in ALLOWED_EXT:
            ...
            add_event("BUFFER", f"Skipped '{fname}' (ext '{_ext}' not allowed)", "warning")
            return
...
async def on_new_message(event):
    ...
    if event.message.action and isinstance(event.message.action, MessageActionRequestedPeerSentMe):
                    ...
                    try:
                        ...
                        if action.button_id == 1:
                            if signed_id in ACTIVE_CHANNELS:
                                await event.reply(f"✅ `{title}` (ID: `{signed_id}`) is already being watched.", buttons=Button.clear())
                                return
                            ACTIVE_CHANNELS.add(signed_id)
                            save_config_to_env({"TARGET_CHANNELS": ",".join(str(ch) for ch in sorted(ACTIVE_CHANNELS))})
                            logger.info(f"[CHANNELS] Added via GUI: {title} ({signed_id})")
                            await event.reply(
                            ...
                            if signed_id not in ACTIVE_CHANNELS:
                                await event.reply(f"⚠️ `{title}` (ID: `{signed_id}`) is not in the watch list.", buttons=Button.clear())
                                return
                            ACTIVE_CHANNELS.discard(signed_id)
                            save_config_to_env({"TARGET_CHANNELS": ",".join(str(ch) for ch in sorted(ACTIVE_CHANNELS))})
                            logger.info(f"[CHANNELS] Removed via GUI: {title} ({signed_id})")
                            await event.reply(
                        ...
                        await event.reply(f"❌ Failed to resolve selection: {ex}", buttons=Button.clear())
                else:
                    await event.reply("⚠️ Please select a channel (broadcast), not a user or group.", buttons=Button.clear())
            return
...
class LogDashboard:
    ...
    def show_dashboard(cls):
        ...
        with cls._lock:
            if cls._instance is not None:
                logger.info("[GUI] LogDashboard instance already exists, calling focus()")
                cls._instance.focus()
                return
    ...
    def _run_gui(self):
        ...
        title_label = tk.Label(
            header, text="📡 TG FDM Proxy Event Monitor (Real-time)", 
            font=("Segoe UI", 10, "bold"), bg="#161b22", fg="#58a6ff"
        )
        ...
        self.btn_monitor = tk.Button(
            nav_frame, text="Event Monitor", font=("Segoe UI", 9, "bold"),
            bg="#21262d", fg="#58a6ff", activebackground="#21262d", activeforeground="#58a6ff",
        ...
        self.btn_settings = tk.Button(
            nav_frame, text="Settings", font=("Segoe UI", 9, "bold"),
            bg="#161b22", fg="#8b949e", activebackground="#21262d", activeforeground="#c9d1d9",
            relief=tk.FLAT, bd=0, padx=12, pady=4, cursor="hand2",
        ...
        scrollbar = tk.Scrollbar(self.monitor_frame, bg="#0d1117", activebackground="#30363d")
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        self.text_widget = tk.Text(
            self.monitor_frame, wrap=tk.WORD, yscrollcommand=scrollbar.set,
            bg="#0d1117", fg="#c9d1d9", insertbackground="white",
            selectbackground="#21262d", selectforeground="#c9d1d9",
        ...
        self.text_widget.tag_configure("time", foreground="#8b949e", font=("Consolas", 9))
        self.text_widget.tag_configure("msg", foreground="#c9d1d9", font=("Segoe UI", 9))
        
        # Badges
        self.text_widget.tag_configure("badge_system", foreground="#58a6ff", font=("Consolas", 9, "bold"))
        self.text_widget.tag_configure("badge_receive", foreground="#388bfd", font=("Consolas", 9, "bold"))
        self.text_widget.tag_configure("badge_buffer", foreground="#d29922", font=("Consolas", 9, "bold"))
        self.text_widget.tag_configure("badge_quality", foreground="#39d353", font=("Consolas", 9, "bold"))
        self.text_widget.tag_configure("badge_download", foreground="#56d364", font=("Consolas", 9, "bold"))
        self.text_widget.tag_configure("badge_error", foreground="#f85149", font=("Consolas", 9, "bold"))
        ...
        lbl_l_heading = tk.Label(
            left_pane, text="🔑 Credentials & Network",
            font=("Segoe UI", 10, "bold"), bg="#161b22", fg="#58a6ff"
        )
        lbl_l_heading.grid(row=0, column=0, columnspan=2, sticky=tk.W, padx=15, pady=(15, 10))
        
        # API ID
        tk.Label(left_pane, text="API ID:", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=1, column=0, sticky=tk.W, padx=15, pady=5)
        self.api_id_var = tk.StringVar(value=str(API_ID))
        tk.Entry(left_pane, textvariable=self.api_id_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=1, column=1, sticky=tk.EW, padx=15, pady=5)
        
        # API Hash
        tk.Label(left_pane, text="API Hash:", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=2, column=0, sticky=tk.W, padx=15, pady=5)
        self.api_hash_var = tk.StringVar(value=str(API_HASH))
        tk.Entry(left_pane, textvariable=self.api_hash_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=2, column=1, sticky=tk.EW, padx=15, pady=5)
        
        # Bot Token
        tk.Label(left_pane, text="Bot Token:", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=3, column=0, sticky=tk.W, padx=15, pady=5)
        self.bot_token_var = tk.StringVar(value=str(BOT_TOKEN))
        tk.Entry(left_pane, textvariable=self.bot_token_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=3, column=1, sticky=tk.EW, padx=15, pady=5)
        
        # Proxy Host
        tk.Label(left_pane, text="Proxy Host:", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=4, column=0, sticky=tk.W, padx=15, pady=5)
        self.proxy_host_var = tk.StringVar(value=str(PROXY_HOST))
        tk.Entry(left_pane, textvariable=self.proxy_host_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=4, column=1, sticky=tk.EW, padx=15, pady=5)
        
        # Proxy Port
        tk.Label(left_pane, text="Proxy Port:", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=5, column=0, sticky=tk.W, padx=15, pady=5)
        self.proxy_port_var = tk.StringVar(value=str(PROXY_PORT))
        tk.Entry(left_pane, textvariable=self.proxy_port_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=5, column=1, sticky=tk.EW, padx=15, pady=5)
        
        # Download Method
        tk.Label(left_pane, text="Download Method:", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=6, column=0, sticky=tk.W, padx=15, pady=5)
        self.download_method_var = tk.StringVar(value=DOWNLOAD_METHOD)
        opt_download_method = tk.OptionMenu(left_pane, self.download_method_var, "sequential", "parallel")
        opt_download_method.config(
            bg="#21262d", fg="#c9d1d9", activebackground="#30363d", activeforeground="#c9d1d9",
            relief=tk.FLAT, bd=0, highlightthickness=0, font=("Segoe UI", 9)
        )
        ...
        # Download Directory
        tk.Label(left_pane, text="Download Dir:", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=7, column=0, sticky=tk.W, padx=15, pady=5)
        self.download_dir_var = tk.StringVar(value=str(DOWNLOAD_DIR))
        tk.Entry(left_pane, textvariable=self.download_dir_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=7, column=1, sticky=tk.EW, padx=15, pady=5)
        
        # Info Label
        lbl_info = tk.Label(
            left_pane, text="ℹ️ Credentials & Proxy updates require\na manual service restart to apply.",
            font=("Segoe UI", 8, "italic"), bg="#161b22", fg="#8b949e", justify=tk.LEFT
        )
        lbl_info.grid(row=8, column=0, columnspan=2, sticky=tk.W, padx=15, pady=(15, 0))
        
        # Right Pane Fields
        lbl_r_heading = tk.Label(
            right_pane, text="🔍 Rules & Filtering",
            font=("Segoe UI", 10, "bold"), bg="#161b22", fg="#58a6ff"
        )
        lbl_r_heading.grid(row=0, column=0, columnspan=2, sticky=tk.W, padx=15, pady=(15, 10))
        
        # Min Size
        tk.Label(right_pane, text="Min File Size (MB):", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=1, column=0, sticky=tk.W, padx=15, pady=5)
        self.minsize_var = tk.StringVar(value=str(MIN_FILE_SIZE_MB))
        tk.Entry(right_pane, textvariable=self.minsize_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=1, column=1, sticky=tk.EW, padx=15, pady=5)
        
        # Quality Wait window
        tk.Label(right_pane, text="Wait Window (secs):", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=2, column=0, sticky=tk.W, padx=15, pady=5)
        self.waitsecs_var = tk.StringVar(value=str(QUALITY_WAIT_SECS))
        tk.Entry(right_pane, textvariable=self.waitsecs_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=2, column=1, sticky=tk.EW, padx=15, pady=5)
        
        # Allowed Extensions
        tk.Label(right_pane, text="Allowed Exts (e.g. .mp4,.mkv):", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=3, column=0, sticky=tk.W, padx=15, pady=5)
        self.allowed_ext_var = tk.StringVar(value=", ".join(sorted(ALLOWED_EXT)))
        tk.Entry(right_pane, textvariable=self.allowed_ext_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=3, column=1, sticky=tk.EW, padx=15, pady=5)
        
        # Target Channels
        tk.Label(right_pane, text="Watched Channels:", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=4, column=0, sticky=tk.W, padx=15, pady=5)
        self.channels_var = tk.StringVar(value=", ".join(str(ch) for ch in sorted(ACTIVE_CHANNELS)))
        tk.Entry(right_pane, textvariable=self.channels_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=4, column=1, sticky=tk.EW, padx=15, pady=5)
        
        # Blocked Keywords
        tk.Label(right_pane, text="Blocked Keywords:", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=5, column=0, sticky=tk.W, padx=15, pady=5)
        self.keyword_block_var = tk.StringVar(value=", ".join(sorted(KEYWORD_BLOCK)))
        tk.Entry(right_pane, textvariable=self.keyword_block_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=5, column=1, sticky=tk.EW, padx=15, pady=5)
        
        # Allowed Keywords
        tk.Label(right_pane, text="Allowed Keywords:", font=("Segoe UI", 9), bg="#161b22", fg="#c9d1d9").grid(row=6, column=0, sticky=tk.W, padx=15, pady=5)
        self.keyword_allow_var = tk.StringVar(value=", ".join(sorted(KEYWORD_ALLOW)))
        tk.Entry(right_pane, textvariable=self.keyword_allow_var, bg="#21262d", fg="#c9d1d9", insertbackground="white", relief=tk.FLAT, font=("Consolas", 9)).grid(row=6, column=1, sticky=tk.EW, padx=15, pady=5)
        ...
        with EVENT_LOG_LOCK:
            for ev in EVENT_LOG:
                self._add_event_to_ui(ev["time"], ev["category"], ev["message"], ev["level"])
    ...
    def _show_monitor(self):
        if self.btn_monitor and self.btn_settings and self.settings_frame and self.monitor_frame:
    ...
    def _save_settings_action(self):
        ...
        try:
            ...
            if not api_id_raw.isdigit():
                self.save_status_label.config(text="❌ API ID must be a valid integer.", fg="#f85149")
                return
            ...
            if not new_api_hash:
                self.save_status_label.config(text="❌ API Hash cannot be empty.", fg="#f85149")
                return
                
            # Bot Token
            new_bot_token = self.bot_token_var.get().strip()
            if not new_bot_token:
                self.save_status_label.config(text="❌ Bot Token cannot be empty.", fg="#f85149")
                return
                
            # Proxy Host
            new_proxy_host = self.proxy_host_var.get().strip()
            if not new_proxy_host:
                self.save_status_label.config(text="❌ Proxy Host cannot be empty.", fg="#f85149")
                return
                
            # Proxy Port
            proxy_port_raw = self.proxy_port_var.get().strip()
            if not proxy_port_raw.isdigit():
                self.save_status_label.config(text="❌ Proxy Port must be an integer.", fg="#f85149")
                return
            new_proxy_port = int(proxy_port_raw)
            if not (1 <= new_proxy_port <= 65535):
                self.save_status_label.config(text="❌ Proxy Port must be between 1 and 65535.", fg="#f85149")
                return
            ...
            try:
                ...
                self.save_status_label.config(text="❌ Min file size must be > 0.", fg="#f85149")
                return
            ...
            try:
                ...
                self.save_status_label.config(text="❌ Wait window must be >= 0.", fg="#f85149")
                return
            ...
            new_kw_block = {w.strip().lower() for w in block_raw.split(",") if w.strip()}
            
            # Allowed Keywords
            allow_raw = self.keyword_allow_var.get().strip()
            new_kw_allow = {w.strip().lower() for w in allow_raw.split(",") if w.strip()}
            ...
            if new_download_method not in ("sequential", "parallel"):
                self.save_status_label.config(text="❌ Download Method must be sequential or parallel.", fg="#f85149")
                return

            # Download Directory Validation
            new_download_dir = self.download_dir_var.get().strip()
            if not new_download_dir:
                self.save_status_label.config(text="❌ Download Directory cannot be empty.", fg="#f85149")
                return
                
            # 2. Check if restart is required
            restart_required = False
            if (new_api_id != API_ID or new_api_hash != API_HASH or 
                new_bot_token != BOT_TOKEN or new_proxy_host != PROXY_HOST or 
            ...
            config_dict = {
                ...
                "MIN_FILE_SIZE_MB": f"{MIN_FILE_SIZE_MB:.1f}" if MIN_FILE_SIZE_MB.is_integer() else str(MIN_FILE_SIZE_MB),
            ...
            if ok:
                if restart_required:
                    ...
                    add_event("SYSTEM", "Settings saved to .env (restart required for API/Proxy settings)")
                else:
                    self.save_status_label.config(text="✅ Settings saved & applied dynamically!", fg="#39d353")
                    add_event("SYSTEM", "Configuration settings updated dynamically")
            else:
                self.save_status_label.config(text="❌ Settings applied in memory, but failed to write to .env.", fg="#d29922")
...
async def main():
    ...
    add_event("SYSTEM", f"Detected download managers: {', '.join(INSTALLED_MANAGERS.keys()) if INSTALLED_MANAGERS else 'None'}")
    ...
    while True:
        try:
            ...
            logger.warning(f"Connection to Telegram failed: {e}. Retrying in 10 seconds...")
            add_event("SYSTEM", f"Telegram connection failed: {e}. Retrying in 10s...", "warning")
    ...
    if port != PROXY_PORT:
        ...
        add_event("SYSTEM", f"Port {PROXY_PORT} in use, using fallback {port}", "warning")
